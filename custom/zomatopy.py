...
class Zomato:
    ...
    def get_categories(self):
        ...
        for category in a['categories']:
            categories.update({category['categories']['id'] : \
    ...
    def get_city_ID(self, city_name):
        ...
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "cities?q=" + city_name, headers= \
        ...
        if len(a['location_suggestions']) == 0:
            ...
            if str(a['location_suggestions'][0]['name']).lower() == \
                    str(city_name).lower():
    ...
    def get_collections(self, city_ID, limit=None):
        ...
        Returns dictionary of Zomato restaurant collections in a city
        ...
        for collection in a['collections']:
            collections.update({collection['collection']['title'] : \
    ...
    def get_cuisines(self, city_ID):
        """
        Takes City ID as input.
        Returns a sorted dictionary of all cuisine IDs and their
        respective cuisine names.
        """
        self.is_valid_city_id(city_ID)
        ...
        for cuisine in a['cuisines']:
            temp_cuisines.update({cuisine['cuisine']['cuisine_id'] : \
    ...
    def get_establishment_types(self, city_ID):
        """
        Takes City ID as input.
        Returns a sorted dictionary of all establishment type IDs and
        their respective establishment type names.
        """
        self.is_valid_city_id(city_ID)
        ...
        if 'establishments' in a:
            for establishment_type in a['establishments']:
                temp_establishment_types.update({establishment_type \
                        ['establishment']['id'] : establishment_type \
                        ['establishment']['name']})

            for establishment_type in sorted(temp_establishment_types):
                establishment_types.update({establishment_type : \
    ...
    def get_nearby_restaurants(self, latitude, longitude):
        ...
        Returns a dictionary of Restaurant IDs and their corresponding
        Zomato URLs.
        """
        try:
            float(latitude)
        ...
        r = (requests.get(base_url + "geocode?lat=" + str(latitude) + "&lon=" \
            + str(longitude), headers=headers).content).decode("utf-8")
        a = ast.literal_eval(r)

        nearby_restaurants = {}
        for nearby_restaurant in a['nearby_restaurants']:
            nearby_restaurants.update({nearby_restaurant['restaurant']['id'] : \
    ...
    def get_restaurant(self, restaurant_ID):
        ...
        restaurant_details.update({"user_rating" : a['user_rating'] \
                                ['aggregate_rating']})
